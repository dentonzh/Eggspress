...
const ShareXButton = ({className, headline, subtitle, siteName, children}: ShareXButtonProps) => {
  ...
  return (
    <a 
      className={className} 
      target="_blank" 
      onClick={() => {if (!clicked) {window.open(href,'targetWindow','toolbar=no,location=0,status=no,menubar=no,scrollbars=yes,resizable=yes,width=600,height=280'); setClicked(true); setTimeout(() => {setClicked(false)}, 500)};}}
      href={href}
    >
      {children}
    </a>
