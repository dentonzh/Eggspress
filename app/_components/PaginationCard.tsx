  ...
  return (
    <div className="flex flex-wrap w-full md:w-[65ch] py-12 justify-center">
      <div className="w-full text-center font-light text-sm mb-2 text-gray-800 dark:text-gray-100">
        {await getString('paginationRangePrefix', 'Displaying posts ')}<span className="font-normal">{startIndex + 1} - {endIndex}</span>{await getString('paginationRangeSuffix', '')}{await getString('paginationTotalCountPrefix', ' of ')}<span className="font-normal">{postCount}</span>{await getString('paginationTotalCountSuffix', ' of ')}
      </div>
      <div className="flex">
        {page > 1 &&
