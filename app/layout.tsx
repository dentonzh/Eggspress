...
export default async function RootLayout({
  ...
  return (
    <html lang="en">
      <body className={`${font.className} flex flex-col duration-200 overflow-x-hidden min-h-screen justify-between ${await getColors('bg', 'ThemeBody', 'slate-800', 'white')}`}>
        <Navigation />
        <div className={`mb-auto ${await getColors('bg', 'ThemeBody', 'slate-800', 'white')}`}>
          <div className={`px-4 xs:px-0 container mb-12 grow bleed-bg ${await getColors('bleed', 'ThemeBody', 'slate-800', 'white')}`}>
            {children}
