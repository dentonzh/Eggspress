...
ChartJS.register(Colors, CategoryScale, LinearScale, LineController, LineElement, PointElement, Title, Tooltip, Legend)
...
const BubbleChart = ({
  ...
  const radii = data.map((x: any) => {
    try {
      return parseInt(x[fields[3]])
    } catch {}
  })
    ...
    datasets: [
      {
        ...
        data: data.map((x: any) => {
          return {
            x: x[fields[1]],
            y: x[fields[2]],
            r: type === 'scatter' ? 3 : normalizeRadius(x[fields[3]], minRadius, maxRadius),
            rOriginal: x[fields[3]],
          }
        }),
        pointRadius: 2,
        yAxisID: 'y',
  ...
  return (
    <div style={{ minHeight: `${(userOptions?.height || 360) * (windowWidth > 640 ? 1 : 0.8)}px` }}>
      {windowWidth > 0 && <Bubble options={options as any} data={parsedData as any} />}
    </div>
  )
}
